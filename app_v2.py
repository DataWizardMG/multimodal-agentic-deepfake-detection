...
import librosa
import subprocess
import json as _json  # for robust ffprobe JSON parsing
...
# sentence_transformers import moved into a guarded block later to avoid bnb/peft chain
...
if not DEMO_MODE:
    try:
        ...
        # Try sentence_transformers only here (guard against bitsandbytes/peft chain)
        try:
...
# ======================
# Shell runner with logging
# ======================
def _run_and_log(cmd: str, log_stem: str) -> Tuple[int, str, str]:
    """
    Run a shell cmd, capture stdout/stderr, write both to LOG_DIR/{log_stem}.out/.err
    Returns (returncode, stdout, stderr)
    """
    try:
        proc = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        out_path = os.path.join(LOG_DIR, f"{log_stem}.out")
        err_path = os.path.join(LOG_DIR, f"{log_stem}.err")
        ...
        err_path = os.path.join(LOG_DIR, f"{log_stem}.err")
        with open(err_path, "w", encoding="utf-8") as fe:
            fe.write(f"Exception running '{cmd}': {e}")
        return 1, "", str(e)
...
# ======================
# IO (robust audio probe + logs)
# ======================
def process_video(file_path: str, task_id: Optional[str] = None) -> Dict:
    # ---- Robust audio probe with JSON
    probe_log = f"{task_id}_ffprobe_audio" if task_id else "ffprobe_audio"
    rc, out, err = _run_and_log(
        f'ffprobe -v error -show_streams -select_streams a -print_format json "{file_path}"',
        probe_log
    )
    has_audio = False
    audio_codec = ""
    if rc == 0 and out.strip():
        try:
            ...
            if has_audio:
                audio_codec = (streams[0].get("codec_name") or "").lower()
        except Exception:
            has_audio = False
    ...
    if has_audio:
        ...
        extract_log = f"{task_id}_ffmpeg_extract_audio" if task_id else "ffmpeg_extract_audio"
        rc2, out2, err2 = _run_and_log(
            f'ffmpeg -y -i "{file_path}" -vn -acodec pcm_s16le -ar 16000 -ac 1 "{audio_path}"',
            extract_log
        )
        if rc2 != 0 or (not os.path.exists(audio_path)):
            # Extraction failed; downgrade has_audio
            has_audio = False
            audio_path = None
            # Log the extraction failure
            err_path = os.path.join(LOG_DIR, f"{extract_log}.err")
            with open(err_path, "a", encoding="utf-8") as fe:
                fe.write(f"\n\nAudio extraction failed for {file_path}. Return code: {rc2}\n")
                fe.write("FFmpeg output:\n")
                fe.write(out2 or "No output")
                fe.write("\nFFmpeg error:\n")
                fe.write(err2 or "No error")

    # ---- Video frames (sample ~1 fps up to 8s)
    ...
    # stash debug paths so the caller can choose to return them
    return {
        'metadata': metadata,
        'keyframes': frames,
        'audio_path': audio_path,
        'debug_audio': {
            'probe_out': os.path.join(LOG_DIR, f"{probe_log}.out"),
            'probe_err': os.path.join(LOG_DIR, f"{probe_log}.err"),
            'extracted_wav': audio_path or "",
            'codec_detected': audio_codec
        }
    }
...
def run_stage_1(task_id, frames, audio_path, metadata, lang: str) -> Dict[str, dict]:
    ...
    # Hardened gate: if a WAV exists, run audio-related agents
    if audio_path and os.path.exists(audio_path) and os.path.getsize(audio_path) > 0:
        ...
        # Log why audio agents are skipped
        print(f"[{task_id}] Skipping audio agents - audio_path valid: {audio_path and os.path.exists(audio_path)}, size: {os.path.getsize(audio_path) if audio_path and os.path.exists(audio_path) else 0}")
    return res
...
async def detect_deepfake(
    ...
    debug_audio: bool = Query(False, description="Return ffprobe/ffmpeg log paths for audio handling")
):
    ...
    data = process_video(video_path, task_id=task_id)
    frames = data['keyframes']; audio_path = data['audio_path']; metadata = data['metadata']

    # HARDEN: trust the actual file presence for audio usage
    metadata.has_audio = bool(audio_path and os.path.exists(audio_path) and os.path.getsize(audio_path) > 0)
    print(f"[{task_id}] has_audio={metadata.has_audio} audio_path={audio_path} size={os.path.getsize(audio_path) if audio_path and os.path.exists(audio_path) else 0}")
    ...
    debug_block = {}
    if debug_audio and isinstance(data.get('debug_audio'), dict):
        debug_block['audio'] = data['debug_audio']

    agents_run = sorted([k for k,v in per.items() if isinstance(v, dict) and "logit" in v])
    ...
    if mode == "fast" or score1 >= STOP_HIGH or (score1 >= STOP_MID and mode != "strict"):
        ...
        payload = {**result.dict(), "explanation_nl": explanation_nl, "decision_rules": rules, "agents_run": agents_run}
        if debug_block:
...
def _heavy_job(task_id: str, video_path: str, params: dict):
    try:
        data = process_video(video_path, task_id=task_id)
        frames = data['keyframes']; audio_path = data['audio_path']; metadata = data['metadata']
        # no need to set metadata.has_audio here since run_stage_1 looks at audio_path existence
