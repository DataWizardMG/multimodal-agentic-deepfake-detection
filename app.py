...
# ======================
# Celery Worker Init - Load Models Once
# ======================
@worker_process_init.connect
def load_models(**kwargs):
    global asr, sentiment_pipe, zero_shot, paraphrase_model, recognizer, visual_model, visual_preprocess

    # Lightweight pipelines
    asr = pipeline("automatic-speech-recognition",
                   model="openai/whisper-tiny", device=0)
    sentiment_pipe = pipeline("sentiment-analysis",
                              model="distilbert-base-uncased-finetuned-sst-2-english", device=0)
    zero_shot = pipeline("zero-shot-classification",
                         model="valhalla/distilbart-mnli-12-3", device=0)
    ...
    # EfficientNet with quantization
    weights = EfficientNet_B4_Weights.IMAGENET1K_V1
    visual_model = efficientnet_b4(weights=weights)
    visual_model = quantize_dynamic(visual_model, {torch.nn.Linear}, dtype=torch.qint8)
    visual_model = visual_model.to("cuda").eval()
...
def visual_agent_task(task_id: str, frames: list):
    imgs = torch.cat([visual_preprocess(f).unsqueeze(0) for f in frames], dim=0).to("cuda")
    with torch.no_grad():
...
async def detect_deepfake(file: UploadFile = File(...), script: UploadFile | None = File(None)):
    ...
    #dispatch_agents.apply_async(args=[task_id, frames, audio_path, metadata.dict()], task_id=task_id)
