...
def mp_mouth_aspect_ratio(frame: np.ndarray) -> Optional[float]:
    ...
    Approx MAR with MediaPipe:
      vertical: distance(13,14)  (upper/lower inner lip midpoints)
      horizontal: distance(61,291) (mouth corners)
...
def dlib_mouth_aspect_ratio(frame: np.ndarray) -> Optional[float]:
    """Fallback MAR using dlib 68 points: use 51 (top lip) and 57 (bottom) normalized by mouth width (48,54)."""
    if not DLIB_OK:
        return None
    ensure_landmark_model()
    bbox, shape = dlib_face_bbox_and_shape(frame)
...
# EfficientNet-B4 for OOD-ish cue
...
def build_natural_language_explanation(per: dict, label: str, score: float, top_contribs: list[str]) -> str:
    ...
    return (
        ...
        f"Interpretation: higher logits ⇒ more likely fake. "
...
def visual_agent_task(task_id: str, frames: List[np.ndarray]):
    ...
    Multi-cue visual prior (frame-only), Stage-1:
      - CLIP zero-shot prior: P("AI-generated image")
      - GAN-like FFT peakiness (radial power 'peakiness')
      - Facial symmetry (1 - SSIM of left vs mirrored-right) on face crop
      - ImageNet OODness proxy: 1 - max softmax from EfficientNet
    """
    ...
    for f in frames:
        ...
        # --- Facial symmetry (use uint8 so SSIM doesn't need data_range)
        ...
        if h > 8 and w > 8:
            try:
                s = ssim(left[:h, :w], right_m[:h, :w])  # dtype=uint8 → OK
        ...
        # --- EfficientNet OODness: 1 - max softmax
        try:
...
def audio_anti_spoof(y: np.ndarray, sr: int) -> float:
    """
    Very lightweight anti-spoof proxy in [0,1]:
...
def audio_agent_task(task_id: str, audio_path: str):
    """
    Audio-only cue (fast):
      - SpeechBrain x-vector embedding norm
      - Spectral flatness
      - Lightweight anti-spoof score
    Returns higher logit => more likely fake.
    """
    t0 = datetime.utcnow().timestamp()
    ...
    return AgentResponse(
        agent='audio',
        logit=logit,
        evidence={'embedding_norm': norm, 'rms': rms, 'spectral_flatness': flat, 'anti_spoof': spoof},
...
def text_agent_task(task_id: str, audio_path: str, lang: str):
    """
    Transcript → sentiment (XLM-R) + zero-shot NLI (XLM-R) + optional script deviation (multilingual SBERT)
    """
    # ASR with optional language hint
    gen_kwargs = {}
    if lang in {"hi", "en"}:
        # Whisper expects language code (e.g., "hi", "en")
        gen_kwargs = {"language": lang}
    asr_out = asr(audio_path, generate_kwargs=gen_kwargs) if gen_kwargs else asr(audio_path)
    transcript = asr_out['text']

    # Sentiment (supports Hindi & English)
    ...
    if original:
        ...
        dev = 1 - float(util.cos_sim(emb1, emb2).item())
...
def run_stage_1(task_id, frames, audio_path, metadata, lang: str) -> Dict[str, dict]:
    ...
    if metadata.has_audio and audio_path:
        ...
        # expose anti-spoof score separately too (for transparency)
        # already folded inside audio logit via evidence['anti_spoof']
    return res
...
async def detect_deepfake(
    ...
    rules = []
    if label1 == "uncertain" and (mode == "strict" or force_decision or mode == "fast"):
        label1, rules = resolve_uncertain_with_tiebreakers(per, score1, real_cut, fake_cut)
    ...
    rules = []
    if label2 == "uncertain" and (mode == "strict" or force_decision):
        label2, rules = resolve_uncertain_with_tiebreakers(per, score2, real_cut, fake_cut)
...
if __name__ == "__main__":
    import uvicorn
    print("MediaPipe available:", MP_OK, "| dlib available:", DLIB_OK)
